    ...
    <div class="sidebar-menu-con" :style="{width: shrink ? '60px' : '230px', overflow: shrink ? 'visible' : 'auto'}">
      <shrinkable-menu :shrink="shrink" :menu-list="menuList" :open-names="openNames" @on-change="handleChange">
        <div class="logo-con" slot="top">
          <span v-if="shrink" style="line-height: 60px; font-weight: 800; font-size: 20px; color: white;">业绩</span>
          <span v-if="!shrink" style="line-height: 60px; font-weight: 800; font-size: 20px; color: white;">业绩管理系统</span>
        </div>
      </shrinkable-menu>
    </div>
