...
class LoonFlowTicketViewSet(ViewSet):
    ...
    def list(self, request, format='json'):
        ...
        # print("index:-->",index)
        select_name = request.query_params.get('select_name', None)
        ...
        # print(category)
        newPassword = request.query_params.get('newPassword', None)
        password =request.query_params.get('password', None)
        url = '/api/v1.0/tickets?username={}'.format(username)

        if (not select_name) and category == 'pwd':
            # 不去6060，那么url就不需要构造了，直接在下面修改密码
            # url += '&category={}&password={}&newPassword={}&username={}'.format(category,password,newPassword,username)
            # print(newPassword)
            '''
            update shutongsql's pwd
            '''
            from account.models import ShutongUser
            # print(ShutongUser.objects.all())
            '''
            ‘原密码验证’以后写
            '''
            user = ShutongUser.objects.filter(username=username).first()
            user.set_password(newPassword)
            user.save()
            '''
            # user = ShutongUser.objects.filter(username='webb').first()
            # user.set_password('000000s')
            # user.save()
            '''
        if (not select_name) and category != 'pwd':
            ...
            print("category")
            # print(url)
        if category and select_name:
            # print("select_name")
            url += '&category={}&select_name={}&time_start={}&time_end={}'.format(category,select_name,time_start,time_end)
            # print(url)
            '''
            url请求loonflow6060
            '''
        if index:
            # print("index")
            # print("check param index")
            url += '&category={}&index={}'.format(category,index)
            # print(url)
            '''
            url请求loonflow6060
            '''
