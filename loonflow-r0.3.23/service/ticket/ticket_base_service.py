...
from apps.ticket.models import TicketRecord, TicketCustomField, TicketFlowLog,Year_index
...
from apps.account.models import LoonUserManager
...
class TicketBaseService(BaseService):
    ...
    def get_ticket_list(cls, sn='', title='', username='', create_start='', create_end='', workflow_ids='', state_ids='', ticket_ids= '', category='', reverse=1, per_page=10, page=1, app_name='', is_end='', is_rejected=''):
        ...
        print("ticket_result_object_list:",ticket_result_object_list)
        ...
        for ticket_result_object in ticket_result_object_list:
            ...
            print(ticket_result_object)
        print(ticket_result_restful_list)
        return ticket_result_restful_list, dict(per_page=per_page, page=page, total=paginator.count)
    '''
    新增api 查询某人的时间段ticket
    '''
    @classmethod
    @auto_log
    def get_ticket_select(cls,creator='', sn='', title='', username='', create_start='', create_end='', workflow_ids='', state_ids='', ticket_ids= '', category='', reverse=1, per_page=10, page=1, app_name='', is_end='', is_rejected=''):
        """
        工单列表
        :param sn:
        :param title:
        ...
        if category == 'owner':
            ...
            if category != 'all':
                ...
                # print("oops!")
                return
            # print('flag all here-->',creator)
            query_params &= Q(creator=creator)
            '''
            # print(query_params)
            # (AND: ('is_deleted', False), ('workflow_id__in', [1, 2, 3, 8]), ('gmt_created__gte', '2020-02-04'), ('gmt_created__lte', '2020-02-18'), ('creator', 'webb'))
            '''
            # ticket_objects = TicketRecord.objects.filter(pub_date__range=(start_date, end_date))
            ticket_objects = TicketRecord.objects.filter(query_params).order_by(order_by_str)
    ...
    def get_ticket_index(cls):
        ...
        '''
        # ticket_objects = TicketRecord.objects.filter(pub_date__range=(start_date, end_date))
        # ticket_objects = Year_index.objects.filter(query_params).order_by(order_by_str)
        # ticket_objects = Year_index.objects.all().annotate(fv = Sum("float_value"))
        # (time.strftime("{}-12-31 23:59:59",this_year)) > "gmt_modified" > (time.strftime("{}-01-01 00:00:00", this_year))
        import time
        this_year = int(time.strftime('%Y',time.localtime(time.time())))
        # print(this_year)
        # print(('%s-01-01 00:00:00.000000' %this_year))
        ticket_objects = Year_index.objects.filter(Q(gmt_created__gte=('%s-01-01 00:00:00.000000'%this_year))&Q(gmt_created__lte=('%s-12-31 23:59:59.000000' %this_year))).values("creator").annotate(fv=Sum("float_value")).values("creator","fv")
        # print(ticket_objects)
        per_page = 10
        page = 1 
        paginator = Paginator(ticket_objects, per_page)
        ...
        ticket_result_object_list = ticket_result_paginator.object_list
        print("ticket_result_object_list:1",ticket_result_object_list)

        ticket_result_restful_list = []
        for ticket_result_object in ticket_result_object_list:
            # print("ticket_result_object2",ticket_result_object)
            # ticket_format_obj = ticket_result_object.get_dict()

            ticket_result_restful_list.append(ticket_result_object)
            # print("ticket_result_object2",ticket_result_object)

        print("---ticket_result_object_list3:",ticket_result_object_list)
    ...
    @classmethod
    @auto_log
    def get_update_newpwd(cls,username='',password='',newpassword=''):
        """
        """
        ...
        print(password)
        print(newpassword)
        LoonUserManager.create_user(self,email="60@qq.com", username='227test', password="111")
        LoonUserManager.update_pwd(self,username=username,password=newpassword)
        '''
        # 或者
    ...
    def new_ticket(cls, request_data_dict, app_name=''):
        ...
        if not update_ticket_custom_field_result:
            return False, msg
        # print("-flag--")
        # print(request_data_dict.items())
        # print("-2--")
        # print(request_data_dict_allow)
        # print("-3--")
        # 新增流转记录
        ## 获取工单所有字段的值
    ...
    def get_ticket_custom_field_value(cls, ticket_id, field_key):
        ...
        # print("ticket_custom_field_obj：",ticket_custom_field_obj)
        if not ticket_custom_field_obj:
            # print("if not ticket_custom_field_obj:pass")
            # 因为有可能该字段还没赋值
            value = None
        else:
    ...
    def add_test(cls, kwargs):
        ...
        # print(kwargs)
        # print(**kwargs)
        new_ticket_flow_log = Year_index(**kwargs)
    ...
    def handle_ticket(cls, ticket_id, request_data_dict, by_timer=False, by_task=False, by_hook=False):
        ...
        if destination_state.type_id == CONSTANT_SERVICE.STATE_TYPE_END:
            import time
            print("heihei:")
            # print(TicketRecord.objects.filter(is_deleted=0).all())
            # import time
            # 格式化成2016-03-20 11:45:39形式
            # print (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
            ticket_Year_index_dict1 = dict(creator=ticket_obj.creator, float_value=(ticket_all_data)['jf_'], sn=ticket_obj.sn,gmt_created=ticket_obj.gmt_created,gmt_modified=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),workflow_id=ticket_obj.workflow_id)
            cls.add_test(ticket_Year_index_dict1)
            print("heihei:")
            # ticket_Year_index.objects.all()
            print(Year_index.objects.all())
            print("1heihei1")
        ...
        # ticket_all_data, msg = cls.get_ticket_all_field_value(ticket_id)#1324
