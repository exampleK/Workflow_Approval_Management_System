...
新表 year_index
'''
class Year_index(BaseModel):
    """
    业绩总记录表
    """
    creator = models.CharField(u'标题', max_length=50, blank=True, default='', help_text="创建者")
    ...
    gmt_created = models.DateTimeField('开始时间', default=datetime.datetime.strptime('0001-01-01 00:00:00', '%Y-%m-%d %H:%M:%S'), blank=True)
    gmt_modified = models.DateTimeField('结束时间', default=datetime.datetime.strptime('0001-01-01 00:00:00', '%Y-%m-%d %H:%M:%S'), blank=True)
    float_value = models.FloatField('积分数', default=0.0, blank=True)
